...
\begin{Bem}\label{gruppentensorprodukt}
  Eine abelsche Gruppe $(G,+)$ wird mit der Skalarmultiplikation
  \begin{align*}
    \Z \times G &\longrightarrow G,\\
    (n,g)&\longmapsto \underbrace{g+\dotsb+g}_{\scriptsize\text{$n$-mal}}
  \end{align*}
  zu einem $\Z$-Modul.
  ...
  % ==== (Gruppentensorprodukt ausführlich)
  ...
  %   \end{tikzcd}
  % \end{center}
  % für eine beliebige Gruppe $G'$ und einen 
  % eindeutigen, bilinearen Gruppenhomomorphismus $\phi$
  % kommutiert.
  ...
  Im Folgenden wird für eine Gruppe $G$ das $n$-fache (Gruppen-)Tensorprodukt
  geschrieben als
  \begin{gather*}
    G^{\otimes n}
...
\begin{Def}\label{defkf}
  Für einen Körper $\F$ definiert man wie folgt die 
  \emph{Milnor K-Gruppen}
  \begin{enumerate}[a)]
  \item $\K_0(\F)\coloneqq \Z$
  \item $\K_1(\F)$ ist die multiplikative Gruppe $\Fx$ 
    additiv geschrieben 
    %mit Addition $\p\colon \KoF\times\KoF\to\KoF$
  ...
  \item $\KnF, n\geq 2,$ ist der Quotient 
    ...
    wird auch \emph{Steinbergrelation} genannt.
...
\begin{Lem}\label{identitaetmal}
  ...
  \end{gather*}
  und insbesondere ergibt sich für $a,b\in \Fx$, dass
  $\{a,b\}= \m\{b,a\}$.
  \begin{proof}
...
\begin{Lem}\label{identitaetquadrat}
  ...
  \begin{gather*}
    \{a\}^2 = \{a\}\{-1\} = \{-1\}\{a\}
  \end{gather*}
  und insbesondere $\{a\}^k=\{a\}\otimes\{-1\}^{k-1}$
  für $k\in\N_1$.
  \begin{proof}
    ...
    \end{gather*}
    Die zweite Aussage folgt durch Induktion.
  \end{proof}
\end{Lem}
...
\begin{Lem}
  ...
  \begin{proof}
    ...
    \begin{align*}
      ...
      &x\mapsto 1-x
    \end{align*}
    Die Überzahl der Nichtquadrate um 1 liefert mit einfacher
    Kombinatorik, dass wenigstens ein Nichtquadrat $a$ auf ein Nichtquadrat
    $1-a=b$ abgebildet wird, was die Existenz
...
% ==== (Alternative Definitionen evtl. für Vortrag)
...
\begin{Def}\label{defbewertungsring}
  Ein Integritätsring $R$ mit Quotientenkörper $\Quot(R)=\F$ heißt
  Bewertungsring (kurz BWR), falls für alle $x\in\F\colon x\in R$ oder
  $x^{-1}\in R$.
  Wir sagen auch $R$ ist \emph{Bewertungsring} zum Körper $\F$.
\end{Def}
...
\begin{Def}
  ...
  heißt Bewertungsabbildung oder kurz \emph{Bewertung},
  falls sie für alle $a,b\in\F$ Folgendes erfüllt:
  \begin{enumerate}[(1)]
  \item $v(a)=\infty \Longleftrightarrow a=0$
  \item $v(ab)=v(a)+v(b)$~~(d.h. $v|_{\Fx}$ ist Gruppenhomomorphismus)
  \item $v(a+b)\geq \min\{v(a),v(b)\}$
  \end{enumerate}
  Die Untergruppe $v(\Fx)$ heißt \emph{Bewertungsgruppe}.
\end{Def}
...
\begin{Satz}\label{eigbewertungsring}
  ...
  \item Die Ideale von $R$ sind total geordnet bzgl. 
    ...
    Für das Maximale Ideal $\frakm$ wird der Körper $R/\frakm$
    \emph{Restklassenkörper} von $R$ genannt.
  ...
  \begin{proof}
    ...
    \item Wir zeigen, dass für zwei Ideale $I\neq J\subset R$ bereits
      eines das andere enthalten muss. Es gebe also ohne Einschränkung ein $x\in I$
      ...
      lokal.
    \item Für die Konstruktion einer solchen Bewertung
      ...
      Ohne Einschränkung $xR\setminus yR\neq \varnothing$ 
      und $xa\in xR\setminus yR, a \in R$. 
      Dann ist für beliebiges $b\in R$ wie oben 
    ...
    \item Dass ein entsprechendes $v$ aus ii) bereits eindeutig
      durch $v|_{R}$ definiert ist, folgt daraus, dass für alle
      $x\in\F$ wegen $R$ Bewertungsring entweder
      $x\in R$ oder $x^{-1}\in R$ und damit
      \begin{gather*}
        v(x)+v(x^{-1}) = v(x\cdot x^{-1}) = v(1) = R  =
        \text{Nullelement in }\Gamma
      \end{gather*}
      weshalb für beliebiges $x\in\Fx$ bereits $v(x)$ oder
      $v(x^{-1})=-v(x)$ definiert ist.
    \end{enumerate}
  \end{proof}
\end{Satz}
...
wenn man eine Bewertung $v$ auf $\F$ bzw. $R$ findet,
wie wir jetzt sehen werden.

\begin{Satz}\label{bewertungsringausbewertung}
  Sei $\F$ ein Körper mit einer Bewertung
...
Bewertung mit demselben Bewertungsring, wie wir im Beispiel
sehen werden.
...
\begin{Bsp}
  ...
  \begin{align*}
    ...
    eine Bewertung, denn $x\mapsto 3x$ ist ein mit der
    Ordnung verträglicher Gruppenisomorphismus zwischen $\Z$ und $3\Z$.
\end{Bsp}

Um eine Eindeutigkeit der Bewertung, die einen
...
\begin{Def}
  Eine Bewertung $v\colon\F\to\Gamma\cup\{\infty\}$
  ...
  \begin{proof}
    ...
    \item[ii)$\Rightarrow$i)] Im Hauptidealring $R$ schreibe das 
      ...
      \begin{gather*}
        ...
        = i+j =v(a)+v(b)
      \end{gather*}
      und wegen (ohne Einschränkung $i\leq j$)
...
\begin{Bem}\label{darstellungdbk}
  ...
  \item Die Bewertung $v\colon R\to\Z\cup\{\infty\}$ zu einem diskreten
    ...
    \begin{gather*}
      ...
      \cong 
      \{R\subset \F\mid \text{$R$ diskreter BWR auf $\F$}\}
    \end{gather*}
  \end{enumerate}
\end{Bem}
...
Bewertung $v\colon\F\to\Z\cup\{\infty\}$ und
...
\begin{Bem}\label{bemhomomorphismen}
  ...
  \item Das Zahme Symbol ist surjektiv. 
    ...
    gilt $R_v\setminus\frakm_v=\Rx_v$, d.h. 
    \begin{gather*}
      \Rx_v=R_v\setminus\frakm_v
...
\begin{Satz}[Spaltende exakte Milnor-Folge]\label{milnorfolge}
...
\begin{Bem}
  ...
  \end{gather*}
  Dies entspricht der Zerlegung eines
...
\begin{proof}[Beweis zu \ref{milnorfolge}]
  ...
  \begin{center}
    ...
    \end{tikzcd}
  \end{center}
  Wir werden nacheinander in den folgenden Lemmata
  \ref{lem1}, \ref{lem2} und \ref{lem3} zeigen, dass gilt
  \begin{align}
  ...
  \end{align}
  sowie zuletzt die Gruppenisomorphie
  \begin{gather} \label{3}
    L_d/L_0\cong \bigoplus_{k=1}^d L_k/L_{k-1} \,,
  \end{gather}
  Diese drei Aussagen liefern dann insgesamt die Isomorphie
  \begin{align*}
    L_d/L_0
    ...
    \K_{n-1}(\Ftpi)
  \end{align*}
  und zusammengesetzt für jedes $d\in\N_1$ die exakte Sequenz
  \begin{center}
    \begin{tikzcd}
  ...
  \begin{center}
    \begin{tikzcd}
      0\ar[r] 
      &\displaystyle \varinjlim_{d\in\N_1} L_0=L_0\ar[r] \disoarrow
      &\displaystyle \varinjlim_{d\in\N_1} L_d\ar[r] \ar[equal]{d}
      &\displaystyle \varinjlim_{d\in\N_1} (L_d/L_0) \ar[r]\disoarrow
      &0\\
      0\ar[r] 
      &\displaystyle \varinjlim_{d\in\N_1} \KnF=\KnF  \ar[r] 
      &\displaystyle \varinjlim_{d\in\N_1} L_d\ar[r]
      &\displaystyle \varinjlim_{d\in\N_1}
  ...
  \ref{spaltendeexaktesequenz} in \ref{lem1} damit zeigen, dass
...
\end{proof}
  ...
  Retraktion hat, und zum anderen, dass \eqref{2} und \eqref{3}
  gelten.
  
  \begin{Lem}[Aussage \eqref{1}]\label{lem1}
  ...
  \begin{proof}[Beweis Lemma \ref{lem2}]
    ...
    Sei ohne Einschränkung $i=2$ und $\ov g_i=\ov g_2=\ov g_2'\ov g_2''\in(\Ftpi)^\times$.
    d.h. 
    \begin{gather*}
    ...
    für ein $g\in\F[t]$ mit $\deg(g)<d$ die Gleichung
    $g_2=\pi g + g_2'g_2''$ erfüllt ist.
    ...
    Ohne Einschränkung sei $\ov g_2 + \ov g_3=1$, 
    ...
    \begin{gather*}
      \left(
      \{\ov g_1,\dotsc,\ov g_n\},%\in\K_{n-1}(\Ftpi), 
      0, 0,\dotsc %\in\K_{n-1}(\F[t]/(\pi'))\}
      \right)%_{\pi'\in P_d\setminus \{\pi\}}
  ...
  \begin{Lem}[Aussage \eqref{3}]\label{lem3}
  ...
  \begin{proof}[Beweis Lemma \ref{lem3}]
    ...
    \begin{center}
      \begin{tikzcd}
        0\arrow{r} & L_{d-1}/L_{d-2}\cong U_{d-1} \arrow{r}
        & U_{d-1,d}\cong L_d/L_{d-2} \arrow{r} 
        &  U_{d}\cong L_d/L_{d-1} \arrow{r} 
        & 0
      \end{tikzcd}
    \end{center}
  ...
  Damit ist Satz \ref{milnorfolge} vollständig gezeigt.
