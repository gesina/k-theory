...
\begin{Def}[spaltende exakte Sequenz]
  Eine exakte Sequenz von Moduln
  \begin{center}
   \begin{tikzcd}
     0 \arrow[r] & M'\ar[r]&M\ar[r]&M''\ar[r]&0
   \end{tikzcd}
 \end{center}
 heißt spaltend, falls es einen Isomorphismus $M'\oplus M''\cong M$
  ...
  Diese Bedingung ist erfüllt, falls $g\colon M\to M'$ ein Schnitt (auch
  Koretraktion) ist, d.h. falls $g$ einen rechtsinversen Homomorphismus
  $f$ besitzt, so dass $g\circ f=id_{M''}$.
\end{Def}
...
\begin{proof}[Beweis zu \ref{milnorfolge}]
  ...
  $\deg(f_i)\leq d$. Für ein Element $\{f_1,\dotsc,f_n\}\in L_d$ 
  sind die $f_i$ also nur von irreduziblen (d.h. primen) Polynomen mit
  Grad kleiner oder gleich $d$ erzeugt.
  Die $L_d$ ergeben offensichtlich eine Filtrierung
  \begin{gather*}
  ...
  \end{tikzcd}
  \end{center}
  Wir werden nacheinander feststellen, dass 
  \begin{gather*}
    L_0\cong \KnF \qquad\text{und}\qquad
    L_d/L_0\cong \bigoplus_{k=1}^d L_k/L_{k-1}
  \end{gather*}
  Mit $P_d\coloneqq \{
  \pi\in \F[t]\mid \pi\text{ prim, normiert, }\deg(\pi)=d
  \}$
  lässt sich die Isomorphie zeigen 
  $L_d/L_{d-1}\cong \bigoplus_{\pi\in P_d} \K_{n-1}(\F[t]/(\pi))$,
  womit wir haben
  \begin{gather*}
    L_d/L_0
    ...
    \K_{n-1}(\F[t]/(\pi))
  \end{gather*}
  Zusammengesetzt haben wir dann für jedes $d\in\N_1$ die Sequenz
  \begin{center}
  \begin{tikzcd}
  ...
  % Zuerst betrachte den Fall $d=0$.
  % Für ein Primelement $\pi\KnFt$ mit $\deg(\pi)=1$
  % (d.h. $\pi=t-\alpha$ mit $\alpha\in\F$)
  % gilt $\F[t]/(\pi)=\F[t]/(t-\alpha)\cong \F$, 
  % also $\K_n(\F[t]/(\pi))\cong\KnF$.
  % Mithilfe der Spezialisierung $\psi_\pi$ zu $\pi$ 
  % und \ref{bemzahmessymbolhom} erhalten wir die Surjektion
  % \begin{gather*}
  %   \KnFt\to \K_n(\F[t]/(\pi))\cong\KnF
  % \end{gather*}
\end{proof}
