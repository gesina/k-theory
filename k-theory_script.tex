...
\begin{Def}[spaltende exakte Sequenz]\label{spaltendeexaktesequenz}
  ...
  %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! REFERENZ: In abelschen
  %Kategorien folgt aus Koretraktion bereits Retraktion und anders herum
\end{Def}
...
\begin{proof}[Beweis zu \ref{milnorfolge}]
  ...
  \begin{proof}[Beweis Lemma \ref{lem2}]
    ...
    % ===== 
    % Wohldefiniertheit auf dem Quotienten $\K_{n-1}(\Ft)$
    % erhält man aus der Eindeutigkeit der Lifts und
    % daraus, dass $h_\pi$ ein Element 
    % ===== 
    ...
    % =====     
    % Projiziert man das Bild von $\ov H_\pi$ auf $L_d/L_{d-1}$,
    % erhält man einen Homomorphismus 
    % $\ov h_\pi=\pr_{L_d/L_{d-1}}\circ \ov H_\pi$, für den gilt
    % mit $\{\pi,g_2,\dotsc,g_n\}\in L_d/L_{d-1}\subset\KnFt$,
    % $\deg(g_i)<d$:
    % \begin{align*}
    %   \ov h_\pi\circ \ov \partial(\{\pi,g_2,\dotsc,g_n\})
    %   &=\ov h_\pi (\{\ov g_2,\dotsc,\ov g_n\})\\
    %   &= \{\pi,g_2,\dotsc,g_n\}\\
    %   &= \id_{L_d/L_{d-1}}\{\pi,g_2,\dotsc,g_n\}
    % \end{align*}
    % Da sich alle Elemente in $L_d/L_{d-1}$ wie oben beschrieben als
    % Summe solcher Elemente wie $\{\pi,g_2,\dotsc,g_n\}$ darstellen lassen
    
    % Es bleibt zu zeigen, dass unser konstruiertes $h_\pi$ tatsächlich
    % ein Inverses zu $\ov\partial_\pi$ darstellt.
    % Sei $\ov{\{\pi,g_2,\dotsc,g_n\}}\in L_d/L_{d-1}$ eine Restklasse
    % von $L_d/L_{d-1}$. Dann gilt wie bei Wohldefiniertheit beschrieben
    % \begin{gather*}
    %   h_\pi\circ \ov\partial_\pi(\ov{\{\pi,g_2,\dotsc,g_n\}})
    %   =h (\partial_d(\{\pi,g_2,\dotsc,g_n\}))
    %   =h (\{\pi,g_2,\dotsc,g_n\})
    %   =\ov{\{\pi,g_2,\dotsc,g_n\}}
    % \end{gather*}
    % und für $\ov{0}\in L_d/L_{d-1}$ ist es trivial.
    % Also ist die Verknüpfung $h_\pi\circ\ov\partial_\pi$ die Identität
    % für alle $\pi\in P_d$. Andersherum ist offensichtlich 
    % ===== 
  ...
  \begin{Lem}[Aussage \ref{3}]\label{lem3}
